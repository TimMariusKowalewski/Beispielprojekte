...
public class JOCLSimpleMandelbrot
{
    ...
    /**
     * The minimum x-value of the area in which the Mandelbrot set should be
     * computed
     */
    private float x0 = -2f;

    /**
     * The minimum y-value of the area in which the Mandelbrot set should be
     * computed
     */
    private float y0 = -1.3f;

    /**
     * The maximum x-value of the area in which the Mandelbrot set should be
     * computed
     */
    private float x1 = 0.6f;

    /**
     * The maximum y-value of the area in which the Mandelbrot set should be
     * computed
     */
     ...
     */
    public JOCLSimpleMandelbrot(int width,
                                int height)
    {
        this.sizeX = width;
        this.sizeY = height;
        ...
        imageComponent.setPreferredSize(new Dimension(width,
                                                      height));
    ...
    private void initCL()
    {
        ...
        // Program Setup
        String source
               = readFile("src/opencl/SimpleMandelbrot.cl");

        // Create the program
        cl_program cpProgram = clCreateProgramWithSource(context,
                                                         1,
                                                         new String[]
                                                         {
                                                             source
        ...
        // Create the kernel
        kernel = clCreateKernel(cpProgram,
                                "computeMandelbrot",
                                null);

        // Create the memory object which will be filled with the
        // pixel data
        pixelMem = clCreateBuffer(context,
                                  CL_MEM_WRITE_ONLY,
                                  sizeX * sizeY * Sizeof.cl_uint,
                                  null,
                                  null);
        ...
        initColorMap(32,
                     ...
                     Color.GREEN,
                     Color.BLUE);
        colorMapMem = clCreateBuffer(context,
                                     CL_MEM_READ_WRITE,
                                     colorMap.length * Sizeof.cl_uint,
                                     null,
                                     null);
     ...
     */
    private void initColorMap(int stepSize,
                              Color... colors)
    {
    ...
    /**
     * Attach the mouse- and mouse wheel listeners to the glComponent which
     * allow zooming and panning the fractal
     */
    private void initInteraction()
    {
        ...
        imageComponent.addMouseMotionListener(new MouseMotionListener()
        {
            ...
            public void mouseDragged(MouseEvent e)
            {
                ...
                previousPoint.setLocation(e.getX(),
                                          e.getY());
