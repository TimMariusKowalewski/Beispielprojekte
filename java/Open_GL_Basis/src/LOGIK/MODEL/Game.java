...
import RENDER.Display.IAlive;
import RENDER.Display.IClick;
import RENDER.Display.IMouseOver;
import RENDER.Display.IDrawable;
    ...
    {                          
        ...
        {   
            ...
            for(Object _geo: _object_list) {
                ...
                if(_geo instanceof IAlive) {
                    ((IAlive)_geo).onIsAlive();
                }
            
                if(_geo instanceof IClick) {
                    ((IClick)_geo).onClick(Mouse.getX(), Mouse.getY());
                }
        
                if(_geo instanceof IMouseOver) {
                    ((IMouseOver)_geo).onMouseOver(Mouse.getX(), Mouse.getY());
                }
    
                if(_geo instanceof IDrawable) {
                    //GeometricObjectDecorator.draw(_geo2, "");
                    ((IDrawable)_geo).onDraw();
                }
            }
